...
class Ticket:
    ...
    generate_response() -> dict
        Uses the Bard model to generate the text section of the ticket, returning all text sections in a dictionary.
    ...
    @staticmethod
    def clean_text(response: str, is_list: bool = False) -> str:
        """
        Returns a cleaned response, removing unnecessary text generated by Bard.
    ...
    def iterative_refinement(self, text: str, type: str) -> str:
        ...
        text : str
            Text generated by Bard
        type : str
            The section which the text represents. Available options: 'acceptance_criteria' and 'subtasks'.
        ...
        prompt = f"""Ensure the Jira ticket {type} section given by {text} followings the same short, clear and concise structure as the example:\n{example}"""
    ...
    def post_process(self, text: str, type: str) -> str:
        ...
        pp_text: str
        """
        if type == "description" or type == "assumptions":
            pp_text = self.clean_text(text)
        # for accpetance criteria and substasks use text as prompt to Bard again to ensure response matches examples
        elif type == "acceptance_criteria" or type == "subtasks":
            parsed_text = self.clean_text(text, True)
            pp_text = self.iterative_refinement(parsed_text, type)
        else:
            raise ValueError(
                f"Invalid type: {type}. Options include 'description', 'acceptance_criteria', 'subtasks' or 'assumptions'."
            )
